...
#' @param drugdb dsfb
#' @param admdb dfgh
#' @param time.points dgh
#' @param window sdgh
#' @param method dgh
#' @param stash dgh
#' @param cap.values dhg
...
mm1 <- function(
    ...
    if (method == "number.of.days" & stash > 0) {
        return(exp.calc)
    } #' Should be NULL, warning given from mm2
    ...
    last.dates <- exp.calc[, last.potential.purch := max(eksd), by = pnr][
        last.potential.purch == eksd
    ]
    cols.to.keep <- c(
        "pnr",
        "end",
        "last.potential.purch",
        "total.drug",
        "average.drug",
    ...
    if (!is.null(admdb)) {
        ...
        setnames(
            out,
            c("end", "hospitalized.at.end", "h.days"),
            c("time", "hospitalized.at.time", "h.days.since.potential.purch")
        )
        out[,
            exposed.at.time := (time <=
                (last.potential.purch +
                    drug.supply.days +
                    h.days.since.potential.purch))
        ]
        ...
        out[,
            exposed.at.time := (time <=
                (last.potential.purch + drug.supply.days))
        ]
    }
    return(out[])
}
