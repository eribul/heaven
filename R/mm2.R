...
#' @param method werg
#' @param stash werg
#' @param cap.values werg
#' @param pre.window werg
#' @param print.time werg
#' @param verbose bla bla?
#' @return Exposure period in a given time span
#' @author Anders Munch
mm2 <- function(
    drugs,
    drugdb,
    ...
    if (is.null(restr.drugdb)) {
        stop("No data in the specified periods.")
    }
    #' Make periods unique
    unique.drugdb <- restr.drugdb[,
        data.table(
            n.purchases = .N,
    ...
    no.purch2 <- no.purch2[, first.purch := min(eksd), by = pnr][
        first.purch > start
    ][first.purch == eksd][, first.purch := NULL]
    no.purch2[, eksd.end := eksd]
    no.purch2[, eksd := start]
    no.purch2[,
        c("n.purchases", "total.drug", "average.drug") := rep(list(0), 3)
    ]
    unique.drugdb <- rbind(unique.drugdb, no.purch, no.purch2)
    setcolorder(
        unique.drugdb,
        c(
            names(unique.drugdb)[1:2],
            names(unique.drugdb)[length(names(unique.drugdb))],
            names(unique.drugdb)[3:(length(names(unique.drugdb)) - 1)]
        )
    )
    ...
    if (print.time) {
        print(paste(
            "Restricting and find unique purch. data took:",
    ...
    if (!is.null(admdb)) {
        ...
        hos.periods <- merge(
            ...
            admdb[, data.table(pnr, inddto, uddto)],
            by = "pnr",
            all.x = TRUE,
        ...
        hos.periods[
            !is.na(inddto),
            ":="(
                hos.days = cap(uddto, eksd, eksd.end) -
                    cap(inddto, eksd, eksd.end),
                hos.days.after.start = ifelse(
                    pre.period == 1,
        ...
        hos.periods[
            pre.period == 1 & is.na(hos.days.after.start),
            hos.days.after.start := 0
        ]
        hos.periods.unique <- hos.periods[,
            data.table(
                h.days = sum(hos.days),
                h.days.after.start = sum(hos.days.after.start),
                non.h.days = days.in.period[1] - sum(hos.days),
                non.h.days.after.start = (eksd.end - start)[1] -
                    sum(hos.days.after.start),
        ...
        if (print.time) {
            print(paste(
                "Finding hospitalization periods took:",
                as.numeric(proc.time()[3]) - t
            ))
        }
    } else {
        all.data <- unique.drugdb
        all.data[, ":="(h.days = 0, non.h.days = days.in.period)]
        all.data[
            pre.period == 1,
            ":="(
                h.days.after.start = 0,
    ...
    t <- as.numeric(proc.time()[3])
    all.data[,
        drug.strength.estimate := sapply(average.drug, function(x) {
            discretize(x, drugs$doses$value)
        })
    ]
    all.data[,
        ind := sapply(drug.strength.estimate, function(x) {
            which(x == drugs$doses$value)
        })
    ]
    all.data[,
        c("min.dose", "max.dose", "def.dose") := lapply(
            drugs$doses[-1],
            function(y) y[ind]
        )
    ][, ind := NULL]
    ...
    if (print.time) {
        print(paste("Adding drug info took:", as.numeric(proc.time()[3]) - t))
    }
    ...
    if (method == "number.of.days" & stash > 0) {
        if (verbose) {
            ...
            cat(
                "\nIt is assumed that the patients can have a stash of",
                stash,
                "."
            )
        }
        ...
        if (method == "default.doses") {
            if (verbose) {
                cat("Calculating exposure based on the default dosis only.")
            }
            ...
            all.data[,
                estimated.daily.dose := ifelse(
                    as.numeric(non.h.days) == 0,
                    def.dose,
                    total.drug / as.numeric(non.h.days)
                )
            ]
            if (cap.values) {
                all.data[,
                    estimated.daily.dose := cap(
            ...
            all.data[,
                estimated.daily.dose := ifelse(
                    n.periods > 1,
                    c(
                        estimated.daily.dose[-length(estimated.daily.dose)],
                        max(
                            def.dose[length(estimated.daily.dose)],
                            estimated.daily.dose[
                                length(estimated.daily.dose) - 1
                            ]
                        )
                    ),
                    def.dose[1]
        ...
        all.data[
            ...
            total.drug := pmax(
                0,
                total.drug -
                    estimated.daily.dose * (non.h.days - non.h.days.after.start)
            )
        ]
        ...
        all.data[
            ...
            ":="(
                ...
                non.h.days = non.h.days.after.start
            )
        ][, ":="(
            h.days.after.start = NULL,
        ...
        all.data[
            n.purchases == 0,
            c("estimated.daily.dose", "drug.supply.days") := rep(list(0), 2)
        ]
        #' Also leave out period with 0 days (appears because of the ending dates):
        all.data <- all.data[eksd != eksd.end]
        all.data[, ":="(
            leftover = pmax(0, total.drug - non.h.days * estimated.daily.dose),
            prop.covered.period = as.numeric(
                pmin(days.in.period, h.days + drug.supply.days) /
                    as.numeric(days.in.period)
            )
        )]
        all.data[,
            prop.covered.total := as.numeric(
                sum(pmin(days.in.period, h.days + drug.supply.days)) /
                    as.numeric(sum(days.in.period))
            ),
            by = data.table(pnr)
        ]
    ...
    if (print.time) {
        print(paste("Summarizing took:", as.numeric(proc.time()[3]) - t))
    }
    if (print.time) {
        print(paste("Overall time=", as.numeric(proc.time()[3]) - stoppuhr))
    }
