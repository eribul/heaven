...
medicinMacro <- function(
    ...
    if (NROW(admdb) > 0) {
        ...
        if (id.character.adm != id.character.drug) {
            stop(
                "The id variable ",
                id,
                " should either both be character or both be numeric in input data sets: drugdb and admdb."
            )
        }
    }
    ...
    if (!(type %in% c("dynamic", "period", "cross-sectional"))) {
        stop(
            "Choose either type = \"dynamic\", \"period\" or \"cross-sectional\"."
        )
    }
    ...
    if (type != "dynamic") {
        ...
        if (id != "pnr") {
            setnames(datesdb.work, id, "pnr")
        }
        ## Set default values for relevant arguments
        if (missing(method)) {
            method <- "default.doses"
        }
        if (missing(cap.values)) {
            cap.values <- TRUE
        }
        if (type == "period") {
            if (missing(datesdb.datevars)) {
                datesdb.datevars <- c("start", "end")
            } else if (length(datesdb.datevars) != 2) {
                stop(
                    "When using type=\"period\", there should be two date variable names given in datesdb.datevars"
                )
            }
            ...
            if (!(missing(window))) {
                warning(paste0(
                    "The argument window is ignored when type=\"period\"."
                ))
            }
        }
        if (type == "cross-sectional") {
            ...
            } else if (length(datesdb.datevars) != 1) {
                stop(
                    "When using type=\"period\", there should be one date variable name given in datesdb.datevars"
                )
            }
            if (any(!(datesdb.datevars %in% names(datesdb)))) {
                stop(paste0(
            ...
            if (missing(window)) {
                stop(
                    "window needs to be given when type = \"cross-sectional\"."
                )
            }
            ## Warn against what arguments are not used
            if (!(missing(pre.window))) {
                warning(paste0(
                    "The argument pre.window is ignored when type=\"cross-sectional\"."
                ))
            }
        }
    } else {
        ...
        if (
            ...
            warning(paste0(
                "The arguments datesdb, method, cap.values, window, and pre.window are ignored when type=\"dynamical\"."
            ))
        }
    }
    ...
    if (missing(drugs) || is.null(drugs)) {
        stop("Sorry, no drugs have been specified.")
    }
    if (missing(drugdb) || is.null(drugdb)) {
        stop("No drug purchase data provided")
    }
    if (NROW(admdb) > 0) {
        if (
            any(
                dups <- duplicated(admdb[, c(id, admdb.datevars), with = FALSE])
            )
        ) {
            stop(paste0(
                "Admission data argument 'admdb' has not been prepared correctly:\nThere are duplicated (and/or overlapping) admission periods in at least one person."
            ))
        }
        ptype <- grep("pattype", names(admdb), ignore.case = TRUE, value = TRUE)
        if ((length(ptype) > 0) && any(admdb[[ptype[[1]]]] != 0)) {
            warning(
                "Admission data argument 'admdb' contains admissions that are not overnight hospital admissions, i.e., pattype!=0."
            )
        }
        admdb.work <- copy(admdb)
        if (id != "pnr") {
            setnames(admdb.work, id, "pnr")
        }
    ...
    for (drugname in names(drugs)) {
        ...
        if (id != "pnr") {
            setnames(drugdb.work, id, "pnr")
        }
        if (strength.var != "strnum") {
            setnames(drugdb.work, strength.var, "strnum")
        }
        if (packsize.var != "packsize") {
            setnames(drugdb.work, packsize.var, "packsize")
        }
        if (apk.var != "apk") {
            setnames(drugdb.work, apk.var, "apk")
        }
        if (atc.var != "atc") {
            setnames(drugdb.work, atc.var, "atc")
        }
        ...
        if (id.character.drug) {
            ...
            drugdb.work = merge(
                ...
                by.y = "pnr.db",
                all.x = TRUE
            )[, pnr := tmp.index][, tmp.index := NULL][]
            if (NROW(admdb) > 0) {
                admdb.work.drug = merge(
                    admdb.work.drug,
                    db,
                    by.x = "pnr",
                    by.y = "pnr.db"
        ...
        if (any(dosesmissing)) {
            stop(paste0(
                paste0(
                    "Missing doses (min,max,def) for ",
                    drugname,
                    ": ",
                    paste(
                        unique(drugdb.work[[strength.var]])[dosesmissing],
                        collapse = ", "
                    )
                ),
                "\n"
        ...
        switch(
            ...
            "cross-sectional" = {
                ...
                if (maxdepot != 0) {
                    warning(paste(
                        "\nMethod for calculating with maxdepot > 0 for type different from \"dynamic\" is not implemented yet, so returns NULL for",
                        drugname
                    ))
                } else {
                    processed[[drugname]] <- mm1(
            ...
            "dynamic" = {
                ...
                drugdb.work <- drugdb.work[
                    eksd <= period[2] & eksd >= period[1],
                ]
                ##--- unique id's
                ...
                if (length(idunique) == 0) {
                    ...
                    warning(paste0(
                        "No individual purchased ",
                        paste0(atcs, collapse = ", "),
                        " in this period"
                    ))
                    processed[[drugname]] <- NULL
                } else {
                    if (NROW(admdb) > 0) {
                        admdb.work.j <- admdb.work.drug[
                            inddto <= period[2] & uddto >= period[1],
                        ]
                    } else {
                        admdb.work.j <- data.table(
                            pnr = numeric(0),
                            inddto = numeric(0),
                            uddto = numeric(0)
                        )
                    }
                    if (length(prescriptionwindow) == 0) {
                        prescriptionwindow = 2
                    }
                    if (length(maxdepot) == 0) {
                        stop("Argument max depot missing\n")
                    }
                    if (length(period) == 0) {
                        stop("Argument period missing\n")
                    }
                    ## convert dates to numeric: number of days since 1995-01-01
                    drugdb.work[,
                        eksd := as.numeric(eksd - as.Date("1995-01-01"))
                    ]
                    if (NROW(admdb.work.j) > 0) {
                        admdb.work.j[,
                            inddto := as.numeric(inddto - as.Date("1995-01-01"))
                        ]
                        admdb.work.j[,
                            uddto := as.numeric(uddto - as.Date("1995-01-01"))
                        ]
                    }
                    ...
                    if (length(idunique) > 10000 & splitting == TRUE) {
                        ...
                        for (split in 1:10) {
                            ...
                            out.list[[split]] <- rbindlist(innerMedicinMacro(
                                dat = drugdb.work[split.id],
                                admdat = admdb.work.j,
                                doses = doses,
                                idunique = split.id,
                        ...
                        index[
                            is.na(id.start.adm),
                            ":="(id.start.adm = 0, id.end.adm = -1)
                        ] ## Hack to make cpp generate empty vector instead of 0 scalar
                        for (j in c(
                            "id.start",
                            "id.end",
                            "id.start.adm",
                            ...
                            set(index, j = j, value = index[[j]] - 1)
                        } ## Convert to C++ indexing
                        out <- rbindlist(innerMedicinMacro(
                            dat = drugdb.work,
                            admdat = admdb.work.j,
                            doses = doses,
